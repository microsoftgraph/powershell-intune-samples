...
    [ValidateNotNullOrEmpty()]
	...
	try	{
		# Check that appID is ok 
		$orgMobileApp = MakeGetRequest "mobileApps/$appId" ""
        ...
        Write-Host
        Write-Host "Updating application in Intune..." -ForegroundColor Yellow
		$mobileApp = MakePatchRequest "mobileApps/$appId" ($mobileAppBody | ConvertTo-Json);
...
Update-Win32Lob -SourceFile "$SourceFile" -publisher "Publisher" `
